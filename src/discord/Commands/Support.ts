import {ChannelType,GuildChannel,hyperlink,inlineCode,Team,} from "discord.js";
import { Command, Module, Option } from "../decorator";
import { Twitter } from "./Twitter";
import { db } from "../../db";
import { ownerId } from "../../config/index"
export class Support extends Module {
  @Command({
    name: "insert-twitter-notify",
    permission: "Administrator",
    only: true,
  })
  async insertNotify(
    @Option({ exec: Twitter.allName }) screen_name: string,
    ...
    let notify = db.TwitterNotify.create({
      guildId: channel.guildId,
      targetId: dbUser.id,
      channelId,
    });
  ...
  async deleteNotify(@Option({ exec: Twitter.allNotify }) screen_name: string) {
    ...
    await dbuser.remove();
    return await this.SuccessEmbed(
      `已刪除通知資料 ${inlineCode(screen_name)}`,
      true
    );
  }

  @Command({
    name: "list-twitter-notify",
    permission: "Administrator",
    only: true,
  })
  ...
  async disableNotify(@Option({ required: false }) screen_name?: string) {
    ...
    await user.save();
    return await this.SuccessEmbed(
      `已封鎖 ${inlineCode(screen_name)} 通知`,
      true
    );
  }

  @Command({
    name: "disable-twitter-list",
    permission: "Administrator",
    only: true,
  })
  async disableList() {
    let users = await db.TwitterUser.find({
      select: ["screen_name"],
      where: { enabled: false },
    });
    return await this.SuccessEmbed(
      `已封鎖 ${users.length} 個使用者\n ${users
        .map((x) => x.screen_name)
        .join("、")}`,
      true
    );
  ...
  async report(@Option({ local: "內容" }) msg: string) {
    const user = this.client.users.cache.get(ownerId);
    let check: any;
    if (user.send) {
      check = await user.send({
        embeds: [
          this.Embed.setColor("Aqua").setDescription(msg).setAuthor({
            name: this.i.user.displayName,
            iconURL: this.i.user.avatarURL(),
          }),
        ],
      });
    }
  ...
  async help() {
    await this.SuccessEmbed((x) =>
      x
        .setDescription(`[圖片連結](https://kujyonatsume.github.io/help.jpg)`)
        .setImage("https://kujyonatsume.github.io/help.jpg")
    );
  }

  @Command({ local: "邀請連結", name: "invite" })
  async invite() {
    await this.SuccessEmbed(
      `[邀請連結](https://discord.com/api/oauth2/authorize?client_id=${this.client.user.id}&permissions=8&scope=bot+applications.commands)`,
      true
    );
  }
}

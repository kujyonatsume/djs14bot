...
export class Twitter extends Module {
  ...
  async init() {
    ...
    console.log(`[Twitter Notify]: Init`);
  ...
  private canSendToChannel(ch: GuildBasedChannel) {
    const isTextLike =
  ...
  async Timer() {
    ...
    const tweets = (await Twitter.List.getTweets()).sort((a, b) =>
      toBig(a.id) < toBig(b.id) ? -1 : 1
    );
    ...
    for (const user of dbusers) {
      ...
      const userTweets = tweets.filter(
        (t) => t.author.id === user.id && toBig(t.id) > toBig(user.last_tweet)
      );
      ...
      for (const notify of user.notifys) {
        ...
        const ch = guild?.channels.cache.get(notify.channelId) as TextChannel;
        if (!ch) {
          console.log("找不到此頻道");
          await notify.remove();
          continue;
        ...
        for (const tweet of userTweets) {
          ...
          if (tweet.urls?.length) {
            ...
            if (urlBlock.length > 0)
              embeds[0].addFields({ name: "連結", value: urlBlock });
          }
          ...
          try {
            ...
            if (!tweet.medias || tweet.medias.length === 0) {
              ...
              for (const media of tweet.medias.slice(1, 10)) {
                embeds.push(
                  this.Embed.setColor("Blue").setImage(media.best_url)
                );
              }
              await this.sendWithRetry(ch!, { content, embeds });
            } else {
              await this.sendWithRetry(ch!, { content, embeds });
              const links = tweet.medias
                .map((m) => hyperlink(m.type, m.best_url))
                .join("\n");
  ...
  static async FindOrCreateUser(screen_name: string) {
    ...
    if (!dbUser) {
      ...
      if (!dbUser) {
        ...
        const latest = (await twitterApi.getUserTweets(dbUser.id)).sort(
          (a, b) =>
  ...
  static async guildNotify(
    mod: Twitter,
    i: AutocompleteInteraction,
    current: string
  ) {
    const list = await db.TwitterNotify.findBy({ guildId: i.guildId });
    const users = [...new Set(await Promise.all(list.map((x) => x.target)))]
      .filter((x) => x.screen_name.startsWith(current))
      .map((x) => ({ name: x.screen_name, value: x.screen_name }));
    if (users.length <= 5) return users;
    return users.slice(0, 5);
  }

  static async allGuild(
    mod: Twitter,
    i: AutocompleteInteraction,
  ...
  static async allName(
    ...
    const users = (await db.TwitterUser.findBy({ enabled: true }))
      ...
      .map((x) => ({ name: x.screen_name, value: x.screen_name }));
    if (users.length === 0) return [{ name: key, value: key }];
    if (users.length <= 5) return users;
  ...
  @Command({
    local: "新增通知",
    name: "notify-create",
    desc: "若無符合的帳號請自行輸入 通知身分組請直接加入通知訊息中",
  })
  async notifyCreate(
    @Option({
      local: "帳號",
      description: "若無符合的帳號請自行輸入",
      exec: Twitter.allName,
    })
    screen_name: string,
    @Option({
      local: "通知頻道",
      description: "要讓機器人發送通知的頻道",
      channel_types: [ChannelType.GuildText, ChannelType.GuildAnnouncement],
    })
    channel: TextChannel,
    @Option({
      local: "通知訊息",
      description: "要標註的身分組以及額外要通知的訊息",
      required: false,
    })
    text?: string
  ) {
    ...
    if (!dbUser.enabled)
      return await this.SuccessEmbed(
        `${inlineCode(screen_name)} 已被封鎖`,
        true
      );
    if (
      await db.TwitterNotify.existsBy({
        guildId: this.i.guildId,
        targetId: dbUser.id,
      })
    )
      return await this.SuccessEmbed(
        `伺服器已存在此通知, 如果要更新請使用 ${inlineCode(
          "/twitter notify update"
        )}`,
        true
      );

    const notify = db.TwitterNotify.create({
      guildId: this.i.guildId,
      targetId: dbUser.id,
  ...
  async notifyUpdate(
    ...
    const notify = await db.TwitterNotify.findOneBy({
      guildId: this.i.guildId,
      targetId: id,
    });
    if (notify) {
      ...
      return await this.SuccessEmbed(
        (eb) =>
          eb
            ...
            .addFields(
              { name: "通知頻道", value: `<#${notify.channelId}>` },
              { name: "通知訊息", value: notify.Text ?? "尚未設定" }
            ),
        true
      );
    }
    return await this.SuccessEmbed(
      `伺服器不存在此通知, 如果要新增請使用 ${inlineCode(
        "/twitter notify create"
      )}`,
      true
    );
  }
  ...
  async notifyList(@Option({ local: "頁數", required: false }) page?: number) {
    let users = "",
      tag = "",
      text = "";
    let pageCnt = Math.max((page ?? 1) * 10, 10);
    const list = (
      await db.TwitterNotify.find({ where: { guildId: this.i.guildId } })
    ).slice(pageCnt - 10, pageCnt);
    for (const notify of list) {
      const target = await notify.target;
      users += `${inlineCode(target.screen_name)}\n`;
      const typeTags = (
        JSON.parse(notify.type || '["發佈","引用"]') as string[]
      )
        .map(inlineCode)
        .join(" ");
      tag += `<#${notify.channelId}>/${typeTags}\n`;
    ...
    return await this.SuccessEmbed(
      {
        ...
        fields: [
          ...
          { name: "通知頻道/類型", value: tag, inline: true },
          { name: "通知訊息", value: text, inline: true },
        ],
      },
      true
    );
  ...
  @Command({
    local: "要通知的推文",
    name: "subscribe-notify-type",
    desc: "選擇要通知的推文類型：發佈、轉推、引用、回復 | 預設：發佈、引用",
  })
  async subscribeNotifyType(
    @Option({ local: "帳號", exec: Twitter.guildNotify }) screen_name: string,
    @Option({ local: "發佈", choices: Twitter.YesOrNo, required: false })
    post?: number,
    @Option({ local: "轉推", choices: Twitter.YesOrNo, required: false })
    retweet?: number,
    @Option({ local: "引用", choices: Twitter.YesOrNo, required: false })
    quote?: number,
    @Option({ local: "回復", choices: Twitter.YesOrNo, required: false })
    reply?: number
  ) {
    const { id } = await Twitter.FindOrCreateUser(screen_name);
    const notify = await db.TwitterNotify.findOneBy({
    ...
    if (!notify)
      return await this.SuccessEmbed(
        `伺服器不存在此通知, 請先使用 ${inlineCode("/twitter notify create")}`,
        true
      );
    ...
    return await this.SuccessEmbed(
      `將會搜尋 ${screen_name} ${[...data].map(inlineCode).join(" ")} 的貼文`,
      true
    );
  }
}
